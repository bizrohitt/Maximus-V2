...
**Maximus**
Version 2.0  ·  Place in project root
...
Nothing exists → Phase 0. Backend+Docker but no tools → Phase 1. Tools/downloads but no AI → Phase 2. AI/analytics but no multi-tenancy → Phase 3. Multi-tenancy exists → Phase 4. Custom Admin Dashboard exists → Phase 5.
...
Stage                  Phase 4+ Complete → Custom Admin Dashboard Priority
...
8. Analytics Layer
9. Plugin Marketplace & Multi-Tenant Ecosystem
10. Directories & Advertising
11. Enterprise Features (SSO, White-label, Audit)
...
1. Ollama — local LLM inference (default)
2. LocalAI — OpenAI-compatible local
3. LiteLLM — multi-provider abstraction layer
4. LangChain / Haystack — agent orchestration
5. Qdrant or ChromaDB — vector/embedding storage
...
CMS content with dropdown/modal/tab    Alpine.js
Next.js page / dashboard / data-heavy  React + TypeScript
...
Maximus/                          ← PROJECT ROOT
├── AI_PROJECT_CONTEXT.md         ← THIS FILE (always here)
├── docker-compose.yml            [PHASE 0]
├── docker-compose.prod.yml       [PHASE 0]
├── .env.example                  [PHASE 0]
├── .env.production.example       [PHASE 0]
├── .gitignore                    [PHASE 0]
├── Makefile                      [PHASE 0]  ← ✅ EXISTS
├── README.md                     [PHASE 0]
│
├── backend/                      [PHASE 1]
│   ├── config/settings/ (base/dev/prod/test)
│   ├── apps/
│   │   ├── core/          [P1] base models, mixins, admin_dashboard
...
│   │   ├── ai/            [P3] LLM, agents, workflows, credits, RAG, vector
│   │   ├── directories/   [P5+] curated directories
│   │   ├── marketing/     [P5+] email campaigns
│   │   ├── ads/           [P5+] ad management
│   │   ├── plugins/       [P4/5] plugin registry, marketplace, revenue
│   │   ├── tenants/       [P4] multi-tenancy, RBAC, SSO, white-label
│   │   ├── audit/         [P5+] audit logging
│   │   └── webhooks/      [P5+] outbound webhook system
│   ├── services/ tasks/ utils/ tests/
│
├── frontend/                     [PHASE 1]
│   ├── app/(marketing)/  (marketing pages: tools, downloads, resources, directories, marketplace, ai-tools, onboarding)
│   ├── app/(auth)/       (login, register)
│   ├── app/(dashboard)/  [P2] user dashboard (overview, ai, billing, enterprise, plugins, settings)
│   ├── app/admin/        [P5+] custom admin dashboard
│   ├── components/ui/    (Button, Input, Card, Modal, Badge, ConfettiEffect)
│   ├── components/layout/(Header, Footer, Nav, Sidebar, SearchModal, AnimatedNavbar, GooeyNav)
│   ├── components/marketing/(FAQ, GiftPopupCTA, CallbackPopup, Hero, LogoTicker, QuickActions)
│   ├── components/tools/ (CopyButton, ChatTool, PdfTool — AI tool UIs)
│   ├── lib/api/ hooks/ utils/ validations/ constants/
│   ├── lib/tools-data.ts  (7 AI tool definitions)
│   ├── types/
...
│   ├── blog/ (blog_index_page.html, blog_detail_page.html)
│   └── emails/ (campaign.html)
│
├── nginx/ .github/workflows/     [PHASE 1]
└── docs/ (architecture / api / design-system / deployment / plugin-sdk)
...
☑ Project root folder with name (Maximus)
...
☑ backend/apps/core/admin_dashboard.py (stub for new Custom Admin)
...
☑ backend/apps/downloads/stripe.py + webhook.py + views.py + urls.py
☑ backend/apps/resources/models.py + admin.py
☑ backend/apps/subscriptions/models.py + services.py + stripe.py + urls.py + views.py
☑ backend/apps/leads/models.py + admin.py
☑ backend/apps/directories/models.py + services.py  ← **Phase 5+**
☑ frontend/app/(marketing)/tools/page.tsx
☑ frontend/app/(marketing)/downloads/page.tsx
☑ frontend/app/(marketing)/resources/page.tsx
☑ frontend/app/(marketing)/directories/page.tsx
☑ frontend/app/(marketing)/marketplace/page.tsx
☑ frontend/app/(marketing)/ai-tools/page.tsx
☑ frontend/app/(marketing)/onboarding/page.tsx
...
☑ backend/apps/ai/vector.py (Qdrant + ChromaDB)
☑ backend/apps/ai/credit_system.py
☑ backend/apps/ai/wagtail_ai.py
☑ backend/apps/ai/models.py + admin.py + urls.py + views.py
☑ backend/apps/analytics/models.py + services.py + admin.py
☑ backend/tasks/ai_tasks.py
☑ API key model in subscriptions app

### Phase 4 — Enterprise, Multi-Tenant & Marketplace
☑ backend/apps/tenants/models.py (Tenant, TenantUser, WhiteLabelConfig)
☑ backend/apps/tenants/middleware.py (tenant resolution)
☑ backend/apps/tenants/rbac.py (roles, permissions)
☑ backend/apps/tenants/sso.py (SAML/OIDC placeholders)
☑ backend/apps/plugins/models.py (Plugin, PluginReview, PluginInstallation)
☑ backend/apps/plugins/marketplace.py
☑ backend/apps/plugins/revenue.py
☑ backend/apps/plugins/services.py + urls.py + views.py + admin.py
☑ frontend/app/(dashboard)/enterprise/page.tsx
☑ frontend/app/(dashboard)/plugins/page.tsx
☑ docs/plugin-sdk.md

### Phase 5 — Custom Admin Dashboard (NEW PRIORITY — 2026-06-21)
☑ Admin layout with sidebar navigation (Dashboard, Content, Commerce, AI Tools, Marketing, Advertising, Analytics, Extensions, Settings)
☑ Dashboard overview page with metrics (revenue, orders, users, AI usage)
☑ Django model admin sections for all 19 apps (admin.py + views.py + urls.py + serializers.py)
☑ Design system compliance (Poppins, brand tokens, Tailwind CSS)
☑ Alpine.js interactivity (sidebar toggle, dark mode toggle)
☑ Extension Store foundation (plugin marketplace page in admin)
☐ Wagtail CMS integration within custom admin (link to /cms/ exists, deeper integration pending)
☐ Frontend admin dashboard pages (Next.js) — currently Django templates only

### Phase 6 — Production Polish
☐ Full Stripe webhook integration (subscription events, invoices)
☐ Complete email marketing (Anymail provider config, campaign sending)
☐ User onboarding flow (wizard, feature discovery)
☐ Coolify/Dokploy detailed deployment guide
...
Auth                  JWT: access (15min) + refresh (7d httpOnly)  Security best practice
Base model            Abstract BaseModel with UUID, timestamps    Consistency across all models
Business logic        services.py per app (NOT in views/models)   Clean architecture
...
Brand colors          LOCKED (White / #B5300A / #3ECF8E)          Palette confirmed
Font                  Poppins                                     Brand identity
...
export async function getBlogPost(slug: string): Promise<BlogPost> {
  const res = await apiClient.get<ApiResponse<BlogPost>>(`/api/v1/blog/${slug}/`);
...
<div x-data="{ open: false }">
  <button @click="open = !open" style="color: var(--color-text-primary)">Menu</button>
...
#  Question                          Affects
1  Icon library — Heroicons or Lucide?      All UI components
2  Analytics — PostHog, Umami, or Plausible? Phase 3 analytics app
3  Email provider — Resend or Postal?       Auth emails, lead capture, campaigns
4  Payment region/currency defaults?        Stripe config, subscriptions

(Brand name and color palette are LOCKED)
...
Starting a Session
1. AI reads this file (auto or paste)
2. AI scans the project directory
3. AI runs the Self-Assessment Protocol
4. AI outputs current state summary
5. AI builds the next missing item
...
1. Scan the codebase first
2. Check the phase checklists
3. Build what's missing — in order
4. Follow the coding standards
5. Use design tokens — never hardcode
6. Alpine.js for templates, React for app pages
7. Keep it open-source, self-hostable, license-safe
...
Brand Name: Maximus
Status: LOCKED (Can be changed with explicit approval)

Color Palette (Locked — easily changeable via globals.css tokens)
:root {
  /* Brand Colors */
...
🛠 CUSTOM ADMIN DASHBOARD (NEW DIRECTION — 2026-06-21)

Decision Date: 2026-06-21

Overview
Maximus will have a single unified Custom Admin Dashboard at /admin/. This dashboard will serve as the central control center for the entire platform.

Key Requirements
- One single admin interface (not multiple dashboards)
- Combines Wagtail CMS, Django Admin, and all custom features
- Consistent UI/UX across the entire admin (design system, colors, typography, layout)
- WordPress-like experience
- Built to support a powerful Extension Store in the future

Tech Stack for Custom Admin
- Django + Tailwind CSS + Alpine.js (lightweight)
- Custom templates and layout (not modifying Wagtail core)
...
Status
- Technical specification created (docs/admin-features.md)
- Documentation updated
- Stub exists at backend/apps/core/admin_dashboard.py
